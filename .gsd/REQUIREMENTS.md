...
- Validation: Validated in M005/S06 by fresh launch-hardening proof: `npx --yes supabase db reset --local --yes && pnpm --dir apps/web exec playwright test tests/e2e/calendar-sync.spec.ts`, `npx --yes supabase db reset --local --yes && pnpm --dir apps/web exec playwright test tests/e2e/calendar-shifts.spec.ts`, `npx --yes supabase db reset --local --yes && pnpm --dir apps/mobile exec playwright test tests/e2e/mobile-predictive.spec.ts tests/e2e/mobile-assembly.spec.ts`, and `pnpm build`, which together re-proved the stable web diagnostic seam, zero-new-violation predictive editor accessibility coverage, fresh mobile predictive recurrence/advisory smoke, and green web/mobile build closure for predictive scheduling assistance.
- Notes: Predictive assistance remains intentionally scoped to recurrence suggestions and clash advisories grounded in real schedule data; S06 closed the launch-hardening proof without widening into unrelated AI planning features.
...
### R022 — Previously synced calendars reopen on mobile, and mobile schedule edits survive offline and reconcile cleanly on reconnect.
- Class: continuity
- Status: validated
- Description: Previously synced calendars reopen on mobile, and mobile schedule edits survive offline and reconcile cleanly on reconnect.
- Why it matters: Cross-platform continuity feels fake if the phone app drops the product's offline/local-first contract.
- Source: inferred
...
| R011 | differentiator | validated | M005/S01 | none | Validated in M005/S06 by fresh launch-hardening proof: `npx --yes supabase db reset --local --yes && pnpm --dir apps/web exec playwright test tests/e2e/calendar-sync.spec.ts`, `npx --yes supabase db reset --local --yes && pnpm --dir apps/web exec playwright test tests/e2e/calendar-shifts.spec.ts`, `npx --yes supabase db reset --local --yes && pnpm --dir apps/mobile exec playwright test tests/e2e/mobile-predictive.spec.ts tests/e2e/mobile-assembly.spec.ts`, and `pnpm build`, which together re-proved the stable web diagnostic seam, zero-new-violation predictive editor accessibility coverage, fresh mobile predictive recurrence/advisory smoke, and green web/mobile build closure for predictive scheduling assistance. |
