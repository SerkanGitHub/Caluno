...
function _page($$renderer, $$props) {
  $$renderer.component(($$renderer2) => {
    ...
    MobileShell($$renderer2, {
      ...
      children: ($$renderer3) => {
        $$renderer3.push(`<section class="calendar-stack svelte-7ipbkm" data-testid="calendar-route-state"${attr("data-shell-bootstrap", shellBootstrapMode)}${attr("data-denied-reason", deniedState()?.reason ?? "none")}${attr("data-failure-phase", deniedState()?.failurePhase ?? "none")}${attr("data-attempted-calendar-id", attemptedCalendarId())}>`);
        if (shellFailure()) {
          $$renderer3.push("<!--[1-->");
