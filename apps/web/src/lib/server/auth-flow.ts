export type AuthSurfaceTone = 'neutral' | 'warning' | 'danger';

export type AuthSurface = {
  state: 'ready' | 'signed-out' | 'invalid-session' | 'callback-error' | 'logout-error' | 'auth-required';
  tone: AuthSurfaceTone;
  eyebrow: string;
  title: string;
  detail: string;
...
const FLOW_REASON_COPY: Record<string, string> = {
...
export function extractReasonCode(
...
export function resolveAuthSurface(searchParams: URLSearchParams): AuthSurface {
  ...
  switch (flow) {
    ...
    case 'callback-error':
      return {
        ...
        detail:
          FLOW_REASON_COPY[reasonCode ?? ''] ??
          'The auth callback could not be completed safely. Start a fresh sign-in.',
    ...
    case 'logout-error':
      return {
        ...
        detail:
          FLOW_REASON_COPY[reasonCode ?? ''] ??
          'The auth service did not confirm sign-out. Retry before reusing this browser.',
    ...
    case 'auth-required':
      return {
        ...
        detail:
          FLOW_REASON_COPY[reasonCode ?? ''] ?? FLOW_REASON_COPY.AUTH_REQUIRED,
    ...
    default:
      return {
        ...
        detail:
          'Caluno resolves membership and calendar access on the server first, then renders only the workspace scope your session can prove.',
        reasonCode
      };
  }
}
