...
function readLocalSupabasePublicEnv() {
  ...
  try {
    ...
    throw new Error(
      [
        'Unable to read local Supabase status for Playwright.',
        'Run `supabase db reset --local` before `pnpm -C apps/web playwright test ...`.',
  ...
  if (!url || !publishableKey) {
    throw new Error(
      'Local Supabase status did not expose API_URL and PUBLISHABLE_KEY/ANON_KEY for the Playwright web server.'
    );
  }
